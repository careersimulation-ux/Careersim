    ...
    <header className={`relative z-30 ${transparent ? "" : "border-b border-slate-200/80 bg-white/85 backdrop-blur-xl"}`}>
      <div className="container flex h-20 items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-7 text-sm font-medium text-slate-600 md:flex" aria-label="Primary navigation">
          ...
          {isAuthenticated && <Link href="/portfolio" className="transition hover:text-teal-700">{t.nav.portfolio}</Link>}
        </nav>
        <div className="hidden items-center gap-3 md:flex">
          <LanguageToggle />
          {!loading && (isAuthenticated ? <button type="button" onClick={() => setLocation("/catalog")} className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-800 active:scale-[0.97]"><UserRound className="h-3.5 w-3.5" />{user?.name?.split(" ")[0] ?? t.nav.simulations}</button> : <button type="button" onClick={startLogin} className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-800 active:scale-[0.97]">{t.nav.signIn}<ChevronRight className="h-3.5 w-3.5" /></button>)}
        </div>
