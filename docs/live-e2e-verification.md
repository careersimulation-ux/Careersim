...
## Business Analyst Simulation Verification

The Business Analyst simulation is a second, independent JSON-defined role for Gulf Growth Partners. Its scenario asks the learner to diagnose Jeddah Growth’s stalled opportunity pipeline, synthesize stakeholder and process-audit evidence, and recommend an eight-week workflow pilot. The configuration includes English and Arabic/RTL content, five connected supported task types totaling 100 points, role-specific workspace terminology, and four managed-storage attachments: a real Excel opportunity-pipeline workbook plus three PDF evidence documents.

The live catalog rendered the Business Analyst card as an available intermediate 80-minute simulation alongside Junior Data Analyst, while the obsolete Business Analyst coming-soon card was removed. The Arabic detail route rendered the localized company, story, objectives, and five tasks correctly. Automated validation included configuration registration, a full deterministic 100-point scoring journey, and a cleaned-up live Supabase tRPC workflow that created onboarding, session, five submission, completion, certificate, and portfolio records for a temporary test identity before deleting the test identity and its dependent records. The full suite passed with 21 tests.
