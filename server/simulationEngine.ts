...
const simulationConfigSchema = z.object({
  ...
  files: z.array(z.object({ id: z.string().min(1), name: z.string().min(1), kind: z.enum(["dataset", "report", "memo"]), label: localizedTextSchema, description: localizedTextSchema, usefulFor: z.array(z.string()), content: localizedTextSchema })).min(1),
